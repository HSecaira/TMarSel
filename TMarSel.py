#!/usr/bin/env python3
'''
Script to select gene families as markers for microbial phylogenomics
Version: 0.0
...
import re, lzma, argparse, os, sys, pyfiglet
...
def load_genome_annotations_single_file(fIn, database, compressed):
    """
    ...
    with open_func(fIn, mode='rt') as f:
        for line in f:
            if not line.startswith('#'):
                ...
                tmp.append([orf, float(score), gene, genome])

    df = pd.DataFrame(tmp, columns=['orf', 'bit_score', 'gene_family', 'genome'])
    df.set_index('orf', inplace=True)

    return df

def load_genome_annotations_multiple_files(file_names, database, compressed):
    """
...
def save_marker_orfs(markers_index, genes_mod, filtered_df, genomes_to_keep, output_dir):
    """
    Save ORFs for each marker gene to individual text files.
    ...
    with tqdm(total = len(markers_index), desc = 'Saving progress') as pbar:
        for idx in markers_index:
            marker = genes_mod[idx]
            file_path = os.path.join(output_dir, f'{marker}.txt')
            # Get ORFs for the marker gene
            orfs = filtered_df.query('gene_family == @marker')
...
def main(argv = None):
    ...
    description = f'TMarSel: Tailored Selection gene families as Markers for microbial phylogenomics\nVersion: 0.0\nAuthors: Henry Secaira and Qiyun Zhu\nFebruary 2025\nUsage example: python selection.py -i input_file -db database -k markers -o output_dir\nType python selection.py -h for help')
    args_parser.add_argument('-i', '--input_file', type = str, required = True, help = 'File containing the genome annotations of ORFs into gene families.\nEither a single annotation file OR a file containing a list of annotation file names (one per line)')
    args_parser.add_argument('-input_dir', '--input_dir_files', type = str, help = 'Directory containing the input files. REQUIRED if providing a list of file names in -i).')
    args_parser.add_argument('-db', '--database', type = str, required = True, help = 'Database used for genome annotation: eggnog or kegg')
    args_parser.add_argument('-k', '--markers', type = int, default = 10, help = 'Number of markers to select. Should be less than the number of gene families. Default is 10')
    args_parser.add_argument('-min_markers', '--min_number_markers_per_genome', type = lambda x: float(x) if '.' in x else int(x), default = 1, help = 'Minimum number of markers per genome. Can be a percentage or a number. Genomes with fewer markers than the indicated value are discarded. Default is 1')
    args_parser.add_argument('-th', '--threshold', type = float, default = 1.0, help = 'Threshold for filtering copies of each gene family per genome. Default is 1.0, retaining only the ORFs with the maximum bit score for each gene family and genome.')
    args_parser.add_argument('-p', '--exponent', type = int, default = 0, help = 'Exponent of the power mean (cost function) used to select markers. Default is 0')
    args_parser.add_argument('-o', '--output_dir', type = str, required = True, default = './orfs', help = 'Output directory for the ORFs of each marker')
    args_parser.add_argument('-c', '--compressed', action = 'store_true', help = 'Input file(s) is compressed')
    ...
    # Run TMarSel
    run_TMarSel(fIn, database, threshold, k, p, output_dir, compressed, min_markers, input_dir)
    
    return 0

def run_TMarSel(fIn, database, threshold, k, p, output_dir, compressed, min_markers, input_dir):
    # Load data
    # In case of multiple annotation files
    if input_dir:
        with open(fIn, 'r') as f:
            file_names = [f'{input_dir}/{line.strip()}' for line in f]
        print(f'Loading genome annotation data from {len(file_names)} files...')
        df = load_genome_annotations_multiple_files(file_names, database, compressed)
    else:
        print(f'Loading genome annotation data from a single file...')
        df = load_genome_annotations_single_file(fIn, database, compressed)
    ...
    print('Saving ORFs for each marker gene...')
